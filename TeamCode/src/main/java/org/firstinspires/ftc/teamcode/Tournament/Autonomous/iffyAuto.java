...
@Autonomous(name = "REEEEEEEEEEEEE", group = "Autonomous")
public class iffyAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        robot.rintake.setPower(0);
        robot.lintake.setPower(0);
        robot.setDriveEncoders(1, -1, -1, 1, -strafeDistance/2-100, -strafeDistance/2-100, -strafeDistance/2-100, -strafeDistance/2-100);
        while (robot.fleft.isBusy() && robot.fright.isBusy() && opModeIsActive()) {
            idle();
        }
        robot.brake();
        robot.setDriveEncoders(.4, -.4, -.4, .4, -150, 150, 150, -150);
        while (robot.fleft.isBusy() && robot.fright.isBusy() && opModeIsActive())
            idle();
        robot.brake();
        //Flips the cube up
